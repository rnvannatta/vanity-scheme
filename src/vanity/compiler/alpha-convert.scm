...
(define-library (vanity compiler alpha-convert)
  ...
  (define (alpha-convert expr)
    ...
    (define (combination? x)
      (and (pair? x) (not (memv (car x) '(quote lambda ##qualified-lambda case-lambda ##qualified-case-lambda ##instrinsic ##basic-intrinsic ##foreign.function begin if or letrec ##letrec)))))
    ...
    (define (iter-atom x env)
      ...
      (match x
        ...
        (('##qualified-lambda name static? args body)
         `(##qualified-lambda ,name ,static? . ,(iter-lambda args '() body env)))
        (('case-lambda (args bodies) ...)
         (cons 'case-lambda (map (lambda (arg body) (iter-lambda arg '() body env)) args bodies)))
        (('##qualified-case-lambda name static? (args bodies) ...)
         `(##qualified-case-lambda ,name ,static? . ,(map (lambda (arg body) (iter-lambda arg '() body env)) args bodies)))
      ...
      (if (application? expr)
          (match expr
            ...
            (('##letrec path ((xs vals) ...) body)
             (append `(##letrec ,path)
               (cdr (iter-letrec xs '() body vals env))))
