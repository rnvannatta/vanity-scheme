...
(define-library (vanity compiler cps)
  ...
  (define (to-cps expr paths)
    ...
    (define (iter-let args body vals k)
      (let loop ((appl '()) (args-left args) (vals vals))
       (cond
         ((null? vals)
          (let ((appl (reverse appl)))
            `((lambda ,args ,(iter2 body k)) . ,appl)))
         ((application? (car vals))
          (let ((x (if (pair? args-left) (gensym (ungensym (car args-left))) (gensym "%x"))))
            (iter2 (car vals) `(continuation (,x) ,(loop (cons x appl) (if (pair? args-left) (cdr args-left) args-left) (cdr vals))))))
         (else (loop (cons (iter-atom (car vals)) appl) (if (pair? args-left) (cdr args-left) args-left) (cdr vals))))))
    ...
    ; reduces args to atoms left to right, naming applications, then builds (k atoms)
    (define (iter-args args k)
      (let loop ((atoms '()) (args args))
        (cond
          ((null? args) (k (reverse atoms)))
          ((application? (car args))
           (let ((x (gensym "%x")))
             (iter2 (car args) `(continuation (,x) ,(loop (cons x atoms) (cdr args))))))
          (else (loop (cons (iter-atom (car args)) atoms) (cdr args))))))
    ; whether a lambda used as a continuation can accept argc values
    (define (lambda-cont-arity-ok? formals argc)
      (cond ((symbol? formals) #t)
            ((null? formals) (= argc 0))
            (else (and (> argc 0) (lambda-cont-arity-ok? (cdr formals) (- argc 1))))))
    ; returning atoms to cont, as if by (values . atoms)
    (define (emit-values cont atoms)
      (match cont
        (('continuation (x) body)
         (if (= (length atoms) 1)
             `(,cont . ,atoms)
             (compiler-error "values: returned wrong number of values to a single-value context" (length atoms))))
        (('continuation body) `(,cont . ,atoms))
        (('lambda formals body)
         (if (lambda-cont-arity-ok? formals (length atoms))
             `(,cont . ,atoms)
             (compiler-error "call-with-values: producer returns wrong number of values for consumer" formals (length atoms))))
        (else `(,cont . ,atoms))))
    (define (iter2 expr cont)
      (if (application? expr)
          (match expr
            ...
            (('##vcore.values . xs)
             (iter-args xs (lambda (atoms) (emit-values cont atoms))))
            (('##vcore.call-with-values producer ('lambda cformals cbody))
             (let ((shim `(lambda ,cformals ,(iter2 cbody cont))))
               (cond
                 ((application? producer)
                  (let ((p (gensym "%p")))
                    (iter2 producer `(continuation (,p) (,p ,shim)))))
                 (else
                   (match producer
                     (('lambda () pbody) (iter2 pbody shim))
                     (('lambda (formal . formals) pbody)
                      (compiler-error "call-with-values: producer must accept zero arguments" producer))
                     (else `(,(iter-atom producer) ,shim)))))))
            ((('lambda (x) body) val)
             (if (application? val)
          ...
          (emit-values cont (list (iter-atom expr)))))
