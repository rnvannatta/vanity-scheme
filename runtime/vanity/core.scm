...
(define-library (vanity core)
  ...
  (define (record=? x y)
    (if (not (and (= (record-length x) (record-length y))
                  (eq? (record-ref x 0) (record-ref y 0))))
        #f
  ...
  (define (printout-record-notnice x write? port)
    ...
    (##vcore.display-word "(" port)
    (printout (record-ref x 0) write? port)
    (let loop ((i 1))
      (unless (= i (record-length x))
        (##vcore.display-word " " port)
        (printout (record-ref x i) write? port)
        (loop (+ i 1))))
    (##vcore.display-word ")" port))
  (define (printout-record-nice x write? port)
    (define refl (record-ref x 0))
    (##vcore.display-word "#record" port)
    (##vcore.display-word "(" port)
    (printout (car refl) write? port)
    (let loop ((i 1) (rest (cdr refl)))
      (unless (= i (record-length x))
        (##vcore.display-word " " port)
        (printout (cons (car rest) (record-ref x i)) write? port)
        (loop (+ i 1) ( cdr rest))))
    (##vcore.display-word ")" port))
  (define (printout-record x write? port)
