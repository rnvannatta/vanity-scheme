...
(define-library (vanity compiler cps)
  ...
  (define (application? x)
    (and (pair? x) (not (memv (car x) '(quote lambda ##qualified-lambda ##qualified-case-lambda continuation case-lambda ##foreign.function ##inline)))))
  (define (combination? x)
    (and (pair? x) (not (memv (car x) '(quote lambda ##qualified-lambda ##qualified-case-lambda continuation case-lambda ##foreign.function ##inline begin if or letrec)))))
  (define (to-cps expr)
    (define (iter-atom x)
      ...
      (match x
        ...
        (('##qualified-lambda name args body)
         (cons '##qualified-lambda (cons name (iter-lambda args body))))
        (('case-lambda (args body) ...)
         (cons 'case-lambda (map iter-lambda args body)))
        (('##qualified-case-lambda name (args body) ...)
         (cons '##qualified-case-lambda (cons name (map iter-lambda args body))))
    ...
    (define (iter2 expr cont)
      (if (application? expr)
          (match expr
            ...
            ((('##qualified-lambda name (x) body) val)
             (if (application? val)
                 (iter2 val `(continuation (,x) ,(iter2 body cont)))
                 (iter-let (list x) body (list val) cont)))
            ((('##qualified-lambda name args body) . vals)
             (iter-let args body vals cont))
            (('letrec ((xs vals) ...) body)
    ...
    (define (iter expr)
      (if (= n 0)
          ...
          (match expr
            ...
            (('##qualified-lambda name args body)
             `(##qualified-lambda ,name ,args ,(iter body)))
            (('continuation args body)
             `(continuation ,args ,(iter body)))
            (('case-lambda (args body) ...)
             (cons 'case-lambda (map (lambda (args body) `(,args ,(iter body))) args body)))
            (('##qualified-case-lambda name (args body) ...)
             (cons '##qualified-case-lambda (cons name (map (lambda (args body) `(,args ,(iter body))) args body))))
  ...
  (define (optimize-impl expr inlining?)
    (define (optimize-lambda expr)
      (match expr
        ((qlamb name xs body)
         `(,qlamb ,name ,xs ,(optimize-apply body)))
        ((lamb xs body)
    ...
    (define (optimize-let let-expr)
      (match let-expr
        ...
        ((('##qualified-lambda name (ys ...) expr) xs ...)
         (if (not (= (length ys) (length xs))) (compiler-error "Not enough arguments to lambda"))
         ; TODO switch based on optimization level
         #;(map optimize-atom let-expr)
         (inline-let expr '() '() ys (map optimize-atom xs)))
    ...
    (define (optimize-atom expr)
      (match expr
        ...
        (('##qualified-case-lambda name (args body) ...)
         `(##qualified-case-lambda ,name . ,(map (lambda (args body) `(,args ,(optimize-apply body))) args body)))
        (else expr)))
