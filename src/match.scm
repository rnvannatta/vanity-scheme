...
(define-library "match"
  ...
  (define (gather-variables pattern eqv?)
    (cond ((null? pattern) '())
          ...
          ((vector? (car pattern)) (error "not implemented yet"))
          ((atom? (car pattern))
      ...
      (cond
        ...
        ((symbol? (car pattern-stack))
         `(let ((,(car pattern-stack) ,(car expr-stack)))
            ,(match-iter (cdr expr-stack) (cdr pattern-stack) success-expr)))
        ((vector? (car pattern-stack)) (error "not implemented yet"))
        ((null? (car pattern-stack))
         `(if (##sys.null? ,(car expr-stack))
              ,(match-iter (cdr expr-stack) (cdr pattern-stack) success-expr))
        )
        ((string? (car pattern-stack)) (error "not implemented yet"))
      ...
      `(call/cc
         (lambda (,k)
           ...
           ,(let loop ((patterns (cddr expr)))
              (cond
                ((null? patterns) '((error "no matching case")))
