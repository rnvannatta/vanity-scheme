...
(define-library (vanity compiler transpile)
  ...
  (define (printout2 main purec? debug? shared? static-environments literal-table foreign-functions intrinsics functions qualified-functions declares toplevels)
    ...
    (define (print-literal-other x)
      (printf "VEncodePointer(&~A.sym, VPOINTER_OTHER)" (mangle-symbol x)))
    (define (print-literal-pair x)
      (printf "VEncodePointer(&~A, VPOINTER_PAIR)" (mangle-symbol x)))
    ...
    (define (print-literal x)
      (cond ((integer? x) (printf "VEncodeInt(~Al)" x))
            ...
            (else
              (match x
                (('##string x) (print-literal-other x))
                (('##pair x) (print-literal-pair x))
                (('##vector x) (print-literal-other x))
                (('##typevector x) (print-literal-other x))
    ...
    (define (print-literal-declaration lit)
      (cond ((symbol? (car lit))
            ...
            ((typevector? (car lit))
             (let* ((mangled (mangle-symbol (cdr lit)))
                    (data ((##intrinsic "VCompileTypevector" 2 2) (car lit)))
                    (len (/ (string-length data) 4)))
               ; typevectors use a gensym name, so no weak attributes here
               (printf "static struct { VBlob sym; char bytes[~A]; } ~A = { { .base = { .tag = VBUFFER, .flags = VFLAG_STATIC | VFLAG_IMMUTABLE }, ~A }, \"~A\" };~N"
                (+ len 1) mangled len data)))
             ...
             (if (symbol? (cadar lit))
                 ...
                 (begin
                   ...
                   (printf "VWEAK VClosure _VW_V40~A = { .base = { .tag = VCLOSURE, .flags = VFLAG_STATIC }, (VFunc)~A, NULL };~N"
                     (cdr lit)
                     (cdr lit)))))
            ((and (pair? (car lit)) (eqv? (caar lit) '##pair))
             (let* ((mangled (mangle-symbol (cdr lit)))
                    )
               ; pairs use a gensym name, so no weak attributes here
               (printf "static VPair ~A = { .base = { .tag = VPAIR, .flags = VFLAG_STATIC | VFLAG_IMMUTABLE }, };~N"
                mangled)))
            ((vector? (car lit))
             (let* ((mangled (mangle-symbol (cdr lit)))
                    (len (vector-length (car lit))))
               ; vectors use a gensym name, so no weak attributes here
    ...
    (define (print-dllmain literals functions)
      (define (print-init lit)
        (cond ((symbol? (car lit))
              ...
              ((and (pair? (car lit)) (eqv? (caar lit) '##pair))
               (let ((mangled (mangle-symbol (cdr lit))))
               (printf "  ~A.first = " mangled)
               (print-literal (caadar lit))
               (printf ";\n")
               (printf "  ~A.rest = " mangled)
               (print-literal (cdadar lit))
               (printf ";\n")))
              ((vector? (car lit))
               (do-loop
                 with mangled = (mangle-symbol (cdr lit))
                 for i from 0 below (vector-length (car lit))
                 for x across (car lit)
                 do (printf "  ~A.elems[~A] = " mangled i)
                 do (print-literal x)
                 do (printf ";\n")))
              (else #f)))
      (printf "static __attribute__((constructor)) void ~A() {~N" (gendllmain))
      ; we print symbols first because they're interned, so rather than encoding constant references
      ; to them, later literals like #(a b) need to refer to initialized pointers 
    ...
    (define (print-expr expr args)
      ...
      (match expr
        ...
        (('##inline f . xs)
         (print-inline f xs))
        (('##string x)
         (print-literal-other x))
        (('##pair x)
         (print-literal-pair x))
        (('##vector x)
         (print-literal-other x))
        (('##typevector x)
         (print-literal-other x))
        ...
        (if variadic?
            (if purec?
                (begin
                  ...
                  (printf " self->num_vars = ~A+1;~N" num))
                (begin
                  (printf " VWORD _varargs = VNULL;~N")
