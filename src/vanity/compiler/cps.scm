...
(define-library (vanity compiler cps)
  ...
  (define (combination? x)
    (and (pair? x) (not (memv (car x) '(quote lambda ##qualified-lambda ##qualified-case-lambda continuation case-lambda ##intrinsic ##basic-intrinsic ##foreign.function ##inline begin if or letrec)))))
  (define (to-cps expr)
    (define (iter-atom x)
