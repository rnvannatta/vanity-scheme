...
(define-library "cps"
  ...
  (define (to-cps expr)
    (define (append-reverse xs ys)
      (if (null? xs) ys
          (append-reverse (cdr xs) (cons (car xs) ys))))
    (define (to-cps-lambda lamb)
      (match lamb
        ((args body)
          (let ((k (gensym "k")))
            `((,k . ,args)
               ; the use of quasiquote instead of quote is a horrible hack for a bug somewhere
              ,(let ((x (gensym "x")))
                 (iter (list (cons x body)) (list k x))))))
         (else (error "Not a lambda" lamb))))
    (define (iter-arg arg)
      (match arg
        (('quote _) arg)
        (('lambda . lambda-body) `(lambda . ,(to-cps-lambda lambda-body)))
        (('case-lambda lambda-body ...) `(case-lambda . ,(map to-cps-lambda lambda-body)))
        ((_ . _) (gensym "x"))
        (else arg)))
    (define (iter-subexprs exprs args)
      (let loop ((exprs exprs) (args args))
        (if (null? args) '()
            (match (car exprs)
              (('quote . _) (loop (cdr exprs) (cdr args)))
              (('lambda . _) (loop (cdr exprs) (cdr args)))
              (('case-lambda . _) (loop (cdr exprs) (cdr args)))
              ((_ . _) (cons (cons (car args) (car exprs)) (loop (cdr exprs) (cdr args))))
              (else (loop (cdr exprs) (cdr args)))))))
    (define (iter stack cont)
      (match stack
        (() cont)
        (((y . ('quote x)) . rest)
         (iter rest `((continuation (,y) ,cont) (quote ,x))))
        (((y . ('lambda . lambda-body)) . rest)
         (iter rest `((continuation (,y) ,cont) (lambda . ,(to-cps-lambda lambda-body)))))
        (((y . ('case-lambda lambda-body ...)) . rest)
         (iter rest `((continuation (,y) ,cont) (case-lambda . ,(map to-cps-lambda lambda-body)))))
        (((y . (xs ...)) . rest)
         (let* ((args (map iter-arg xs))
                (newstack (iter-subexprs xs args)))
         (iter (append-reverse newstack rest) `(,(car args) (continuation (,y) ,cont) ,@(cdr args)))))
        (((y . x) . rest) (iter rest `((continuation (,y) ,cont) ,x)))
        (else (error "No matching case" stack))))
    (define (to-cps-impl expr)
      (let ((x (gensym "x")))
        (iter (list (cons x expr)) `(##sys.next ,x))))
    (match expr
      (('##vcore.declare f l)
    ...
    (match expr
      ...
      (('##sys.if k p ('lambda (k1) apply1) ('lambda (k2) apply2))
       (if (or (not (= 1 (ref-count k1 apply1)))
           ...
           (error "failed to inline if statement"))
      ...
      (('##sys.begin k a ('lambda (k1) apply))
       (if (not (= 1 (ref-count k1 apply)))
           (error "failed to inline begin statement"))
      ...
      (('##sys.and k x ('lambda (k1) apply))
       (if (not (= 1 (ref-count k1 apply)))
           (error "failed to inline and statement"))
      ...
      (('##sys.or k x ('lambda (k1) apply))
       (if (not (= 1 (ref-count k1 apply)))
           (error "failed to inline or statement"))
       (optimize-apply `(if ,x (,k ,x) ,(substitute k1 k apply))))

      (('##sys.if . _) (error "failed to inline if statement"))
      (('##sys.or . _) (error "failed to inline or statement"))
      (('##sys.and . _) (error "failed to inline and statement"))
      (('##sys.begin . _) (error "failed to inline begin statement"))
      ; Inlining builtins
