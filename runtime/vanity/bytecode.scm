  ...
  (define (preprocess-vasm tape)
    ...
    (define (preprocess-expr expr pc)
      (match expr
        ...
        (('close x . name)
         (if (and (not (null? name)) (not (and (pair? name) (string? (car name)))))
             (error "vasm close: malformed env name"))
         (cond ((and (symbol? x) (assv x labels)) => (lambda (label) `(close ,(cdr label) . ,name)))
               ((symbol? x) (error "vasm close: unknown label" x))
               ((and (integer? x) (>= x 0)) expr)
               (else (error "vasm close: not a nonnegative integer" x))))
        (('foreign-function x)
         (let ((x (string->symbol (sprintf "~A_shim" x))))
        ...
        (('letrec-begin x . name)
         (if (and (integer? x) (>= x 0)
                  (or (null? name)
                      (and (pair? name) (string? (car name)))))
             expr
             (error "vasm letrec-begin: malformed" expr)))
        (('letrec-end) expr)
