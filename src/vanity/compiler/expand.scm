...
(define-library (vanity compiler expand)
  ...
  (define (expand-syntax expr)
    (match expr
      ...
      (('cut-iter xs args) (expand-syntax `(lambda ,(reverse xs) ,(reverse args))))
      (('cut-iter xs args '<> . rest) (let ((x (gensym "x"))) (expand-syntax `(cut-iter (,x . ,xs) (,x . ,args) . ,rest))))
      (('cut-iter xs args '<...>)
       (let ((rest (gensym 'rest)))
         (expand-syntax
      ...
      (('cute f . args) (expand-syntax `(cute-iter () () . ,(cons f args))))
      (('cute-iter xs args) (expand-syntax `(lambda ,(reverse xs) ,(reverse args))))
      (('cute-iter xs args '<> . rest) (let ((x (gensym "x"))) (expand-syntax `(cute-iter (,x . ,xs) (,x . ,args) . ,rest))))
      (('cute-iter xs args '<...>)
       (let ((rest (gensym 'rest)))
         (expand-syntax
           `(lambda (,@(reverse xs) . ,rest) (apply ,@(reverse args) ,rest)))))
