; Copyright 2023-2024 Richard N Van Natta
;
; This file is part of the Vanity Scheme Compiler.
;
; The Vanity Scheme Compiler is free software: you can redistribute it
; and/or modify it under the terms of the GNU General Public License as
; published by the Free Software Foundation, either version 2 of the
; License, or (at your option) any later version.
; 
; The Vanity Scheme Compiler is distributed in the hope that it will be
; useful, but WITHOUT ANY WARRANTY; without even the implied warranty
; of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the GNU
; General Public License for more details.
;
; You should have received a copy of the GNU General Public
; License along with the Vanity Scheme Compiler.
;
; If not, see <https://www.gnu.org/licenses/>.
;
; This work is published with additional permission, the Vanity Scheme
; Runtime Library Exceptions, which should have been included with the
; Vanity Scheme Compiler.
;
; If not, visit <https://github.com/rnvannatta>

(define-library (vanity compiler lower)
  (import (vanity pretty-print) (vanity core) (vanity list) (vanity compiler utils) (vanity compiler match) (vanity compiler variables) (vanity compiler ffi) (vanity compiler hush) (vanity intrinsics))
  (export bruijn-ify to-functions)
; Strips the names from bound variables and replaces them with indices
; free variables are assumed to be builtin functions
  (define (list-index p l)
    (let loop ((l l) (i 0))
      (cond ((null? l) #f)
            ((p (car l)) i)
            (else (loop (cdr l) (+ i 1))))))

  (define (improper-map f xs)
    (cond
      ((null? xs) '())
      ((pair? xs) (cons (f (car xs)) (improper-map f (cdr xs))))
      (else (f xs))))
  (define (unmangle-formals xs)
    (improper-map ungensym xs))

  (define (bruijn-ify expr debug?)
    (define (lookup depth env x)
      (cond
        ((null? env) x)
        ((list-index (lambda (e) (eqv? x e)) (car env)) => (lambda (idx) (list 'bruijn x depth idx)))
        (else (lookup (+ 1 depth) (cdr env) x))))
    (define (undot lst)
      (if (pair? lst)
          (cons (car lst)
                (undot (cdr lst)))
          (if (null? lst) '()
              (cons lst '()))))
    (define (bruijn-lambda env unmangled-env lamb)
      (match lamb
        (((xs ...) body)
         `(,(length xs) ,(bruijn-iter (cons xs env) (cons (unmangle-formals xs) unmangled-env) body)))
        ((xs body)
         (let ((proper-xs (undot xs)))
          `(,(- (length proper-xs) 1) + ,(bruijn-iter (cons proper-xs env) (cons (undot (unmangle-formals xs)) unmangled-env) body))))
        (else (compiler-error "bruijnify-pass: No matching lambda" lamb))))
    (define (bruijn-iter env unmangled-env expr)
      (match expr
        (('lambda (xs ...) body)
         (define unmangled-formals (unmangle-formals xs))
         `(lambda ,(if debug? `((#f ,unmangled-formals) ,unmangled-env) #f) ,(length xs) ,(bruijn-iter (cons xs env) (cons unmangled-formals unmangled-env) body)))
        (('lambda xs body)
         (define unmangled-formals (unmangle-formals xs))
         (let ((proper-xs (undot xs)))
          `(lambda ,(if debug? `((#f ,unmangled-formals) ,unmangled-env) #f) ,(- (length proper-xs) 1) + ,(bruijn-iter (cons proper-xs env) (cons (undot unmangled-formals) unmangled-env) body))))
        (('##qualified-lambda name static? (xs ...) body)
         (define unmangled-formals (unmangle-formals xs))
         `(##qualified-lambda ,name ,static? ,(if debug? `((,name ,unmangled-formals) ,unmangled-env) #f) ,(length xs) ,(bruijn-iter (cons xs env) (cons unmangled-formals unmangled-env) body)))
        (('##qualified-lambda name static? xs body)
         (define unmangled-formals (unmangle-formals xs))
         (let ((proper-xs (undot xs)))
          `(##qualified-lambda ,name ,static? ,(if debug? `((,name ,unmangled-formals) ,unmangled-env) #f) ,(- (length proper-xs) 1) + ,(bruijn-iter (cons proper-xs env) (cons (undot unmangled-formals) unmangled-env) body))))
        (('case-lambda . cases)
         `(case-lambda ,(if debug? `((#f . ,(map (lambda (e) (unmangle-formals (car e))) cases)) ,unmangled-env) #f) . ,(map (lambda (e) (bruijn-lambda env unmangled-env e)) cases)))
        (('##qualified-case-lambda name static? . cases)
         `(##qualified-case-lambda ,name ,static? ,(if debug? `((,name . ,(map (lambda (e) (unmangle-formals (car e))) cases)) ,unmangled-env) #f) . ,(map (lambda (e) (bruijn-lambda env unmangled-env e)) cases)))
        (('continuation (x) body)
         (define unmangled-formals (list (ungensym x)))
         `(continuation ,(if debug? `((#f ,unmangled-formals) ,unmangled-env) #f) 1 ,(bruijn-iter (cons (list x) env) (cons unmangled-formals unmangled-env) body)))
        (('continuation body)
         ; such continuations are totally ignored, and do not contribute a frame.
         `(continuation ,(if debug? '((#f _) ,unmangled-env) #f) #f ,(bruijn-iter env unmangled-env body)))
        (('letrec ((xs vals) ...) body)
         (define unmangled-formals (unmangle-formals xs))
         `(letrec ,(length xs) ,xs ,(map (lambda (e) (bruijn-iter (cons xs env) (cons unmangled-formals unmangled-env) e)) vals) ,(bruijn-iter (cons xs env) (cons unmangled-formals unmangled-env) body)))
        (('##letrec path ((xs vals) ...) body)
         (define unmangled-formals (unmangle-formals xs))
         `(##letrec ,path ,(length xs) ,xs ,(map (lambda (e) (bruijn-iter (cons xs env) (cons unmangled-formals unmangled-env) e)) vals) ,(bruijn-iter (cons xs env) (cons unmangled-formals unmangled-env) body)))
        (('basic-block cost (xs vals) ... appl)
         (define unmangled-formals (unmangle-formals xs))
         `(basic-block ,cost ,(length xs) ,xs ,(map (lambda (e) (bruijn-iter (cons xs env) (cons unmangled-formals unmangled-env) e)) vals) ,(bruijn-iter (cons xs env) (cons unmangled-formals unmangled-env) appl)))
        (('##foreign.function . _) expr)
        (('quote . _) expr)
        (('##inline f . xs)
         `(##inline ,f . ,(map (lambda (x) (bruijn-iter env unmangled-env x)) xs)))
        (('##qualified-call name static? f . xs)
         `(##qualified-call ,name ,static? ,(bruijn-iter env unmangled-env f) . ,(map (lambda (x) (bruijn-iter env unmangled-env x)) xs)))
        ((f xs ...)
         (cons (bruijn-iter env unmangled-env f) (map (lambda (x) (bruijn-iter env unmangled-env x)) xs)))
        (x
         (if (symbol? x) (lookup 0 env x) x))
        (else (compiler-error "bruijnify-pass: No matching case" expr))))
    (define (doit expr)
      (bruijn-iter '() '() expr))
    (match expr
      (('##foreign.declare . _) expr)
      (('##vcore.declare f l)
       `(##vcore.declare ,f ,(doit l)))
      (else (doit expr))))

  ; replaces lambdas and defines with explicit close statements on functions, and
  ; gathers the functions into a list, and gathers literals into a list and replaces
  ; with references
  (define curlambda 0)
  (define curcont 0)
  (define (to-functions exprs lifting-literals? debug?)
    (define (genlambda fun)
        (set! curlambda (+ curlambda 1))
        (string->symbol (sprintf "~A_V0lambda~A" fun curlambda)))
    (define (gencont fun)
        (set! curcont (+ curcont 1))
        (string->symbol (sprintf "~A_V0k~A" fun curcont)))
    (define qualified-functions '())
    (define foreign-functions '())
    (define intrinsics '())
    (define functions '())
    (define literal-table '())

    (define literal-hushtable (make-hush-table))

    (define static-environments '())

    ; FIXME ASSOC DETECTED
    (define (lookup-literal key)
      (let ((val (hush-table-ref literal-hushtable key (lambda () #f))))
        (if val (cons key val) #f))
      #;(assoc key literal-table))
    (define (register-literal! key val)
      (hush-table-set! literal-hushtable key val)
      (set! literal-table (cons (cons key val) literal-table)))

    (define (lift-intrinsic! sym intrin)
      (if lifting-literals?
          (let* ((key (list '##intrinsic sym))
                 (lookup (lookup-literal key)))
            (if lookup
                (car lookup)
                (let ((lookup (cons key intrin)))
                  (register-literal! key intrin)
                  (car lookup))))
          (list '##intrinsic sym)))
    (define (lift-literal x)
      (if lifting-literals?
          (cond ((integer? x) x)
                ((number? x) x)
                ((##vcore.void? x) x)
                ((##vcore.foreign-pointer? x) x)
                ((char? x) x)
                ((eq? x #t) x)
                ((eq? x #f) x)
                ((null? x) x)
                ((typevector? x)
                 (let ((lookup (lookup-literal x)))
                   (if lookup
                       `(##typevector ,(cdr lookup))
                       (begin
                         (register-literal! x (gensym "typevector"))
                         `(##typevector ,(cdar literal-table))))))
                ((string? x)
                 (let ((lookup (lookup-literal x)))
                   (if lookup
                       `(##string ,(cdr lookup))
                       (begin
                         (register-literal! x (gensym "string"))
                         `(##string ,(cdar literal-table))))))
                ((symbol? x)
                 (if (not (lookup-literal x))
                     (register-literal! x '()))
                 x)
                ((pair? x)
                 (let* ((x (list '##pair (cons (lift-literal (car x)) (lift-literal (cdr x)))))
                        (lookup (lookup-literal x)))
                   (if lookup
                       `(##pair ,(cdr lookup))
                       (begin
                         (register-literal! x (gensym "pair"))
                         `(##pair ,(cdar literal-table))))))
                ((vector? x)
                 (let* ((x (vector-map lift-literal x))
                        (lookup (lookup-literal x)))
                   (if lookup
                       `(##vector ,(cdr lookup))
                       (begin
                         (register-literal! x (gensym "vector"))
                         `(##vector ,(cdar literal-table))))))
                (else (compiler-error "literal-lifting: unknown literal type" x)))
          x))
    (define (iter-lambda fun lamb) 
      (match lamb
        ((n body)
         `(,n ,(iter-apply fun body)))
        ((n '+ body)
         `(,n + ,(iter-apply fun body)))))
    (define (iter-atom fun expr func-position?)
      (match expr
        (('bruijn . _) expr)
        (('lambda debug-info n body)
         (let ((lamb (genlambda fun)))
           (set! functions (cons `(,lamb #t ,(lift-literal debug-info) (,n ,(iter-apply fun body))) functions))
           `(close ,lamb)))
        (('lambda debug-info n '+ body)
         (let ((lamb (genlambda fun)))
           (set! functions (cons `(,lamb #t ,(lift-literal debug-info) (,n + ,(iter-apply fun body))) functions))
           `(close ,lamb)))
        (('##qualified-lambda name static? debug-info n body)
         (let ((lamb (mangle-qualified-function name)))
           (set! qualified-functions (cons (list lamb n #f) qualified-functions))
           (set! functions (cons `(,lamb #t ,(lift-literal debug-info) (,n ,(iter-apply lamb body))) functions))
           (if static?
               `(close ,lamb ,(drop-right name 1))
               `(close ,lamb ))))
        (('##qualified-lambda name static? debug-info n '+ body)
         (let ((lamb (mangle-qualified-function name)))
           (set! qualified-functions (cons (list lamb n #t) qualified-functions))
           (set! functions (cons `(,lamb #t ,(lift-literal debug-info) (,n + ,(iter-apply lamb body))) functions))
           (if static?
               `(close ,lamb ,(drop-right name 1))
               `(close ,lamb))))
        (('case-lambda debug-info . cases)
         (let ((lamb (genlambda fun)))
           (set! functions (cons `(,lamb #t ,(lift-literal debug-info) . ,(map (lambda (e) (iter-lambda fun e)) cases)) functions))
           `(close ,lamb)))
        (('##qualified-case-lambda name static? debug-info . cases)
         (if (null? (cdr cases))
             (iter-atom fun `(##qualified-lambda ,name ,static? . ,(car cases)) func-position?)
             (let ((lamb (mangle-qualified-function name)))
               (set! qualified-functions (cons (list lamb 0 #t) qualified-functions))
               (set! functions (cons `(,lamb #t ,(lift-literal debug-info) . ,(map (lambda (e) (iter-lambda fun e)) cases)) functions))
               (if static?
                   `(close ,lamb ,(drop-right name 1))
                   `(close ,lamb)))))
        (('continuation debug-info n body)
         (let ((k (gencont fun)))
           (set! functions (cons `(,k #t ,(lift-literal debug-info) (,n ,(iter-apply fun body))) functions))
           `(close ,k)))
        (('##foreign.function lang decl ret name . args)
         (let ((mangled (mangle-foreign name)))
           ; FIXME ASSOC DETECTED
           (if (not (assoc mangled foreign-functions))
               (set! foreign-functions (cons (cons mangled expr) foreign-functions)))
           `(##foreign.function ,mangled)))
        (('##intrinsic name . _)
         (if (not (assoc name intrinsics))
             (set! intrinsics (cons (cons name expr) intrinsics)))
         (lift-intrinsic! name name)
         `(##intrinsic ,name))
        (('##basic-intrinsic name . _)
         (if (not (assoc name intrinsics))
             (set! intrinsics (cons (cons name expr) intrinsics)))
         (lift-intrinsic! name name)
         `(##basic-intrinsic ,name))
        (('quote x)
         `(quote ,(lift-literal x)))
        (('##inline f . xs)
         `(##inline ,f . ,(map (lambda (x) (iter-atom fun x #f)) xs)))
        (x (if (symbol? x)
               ; we don't need to create static storage for intrinsics
               ; that are directly applied, only for intrinsics that are passed
               ; as directly applied intrinsics never create null closures
               (if func-position?
                   x
                   (let ((intrin (lookup-intrinsic-name x)))
                     (if (not intrin)
                         x
                         (lift-intrinsic! x intrin))))
               (lift-literal x)))))
    (define (iter-apply fun expr)
      (match expr
        (('define k y x)
         `(define ,k ,(lift-literal y) ,(iter-atom (mangle-symbol y) x #f)))
        (('set! k ('bruijn name . rest) x)
         `(set! ,(iter-atom fun k #f) (bruijn ,name . ,rest) ,(iter-atom (mangle-symbol name) x #f)))
        (('set! k y x)
         `(set! ,(iter-atom fun k #f) ,(lift-literal y) ,(iter-atom (mangle-symbol y) x #f)))
        (('if p x y)
         `(if ,(iter-atom fun p #f) ,(iter-apply fun x) ,(iter-apply fun y)))
        ((('lambda debug-info n body) . xs)
         (let ((lamb (genlambda fun)))
           (set! functions (cons `(,lamb #f ,(lift-literal debug-info) (,n ,(iter-apply fun body))) functions))
           `((close ,lamb) . ,(map (lambda (x) (iter-atom fun x #f)) xs))))
        ((('##qualified-lambda name static? debug-info n body) . xs)
         (let ((lamb (mangle-qualified-function name)))
           (set! qualified-functions (cons (list lamb n #f) qualified-functions))
           (set! functions (cons `(,lamb #f ,(lift-literal debug-info) (,n ,(iter-apply lamb body))) functions))
           `((close ,lamb) . ,(map (lambda (x) (iter-atom lamb x #f)) xs))))
        (('letrec n xs vals body)
         `(letrec ,n ,(map (lambda (x val) (iter-atom (mangle-symbol x) val #f)) xs vals)
            ,(iter-apply fun body)))
        (('##letrec path n xs vals body)
         (if path
             (set! static-environments (cons path static-environments)))
         `(##letrec ,path ,n ,(map (lambda (x val) (iter-atom (mangle-symbol x) val #f)) xs vals)
            ,(iter-apply fun body)))
        (('basic-block cost n xs vals body)
         `(basic-block
            ,cost
            ,n
            ,xs
            ,(map (lambda (x val)
                    (cons (iter-atom fun (car val) #t)
                          (map (lambda (x) (iter-atom fun x #f))
                               (cdr val))))
                  xs
                  vals)
            ,(iter-apply fun body)))
        (('##qualified-call name static? f . xs)
         `(##qualified-call ,name ,static? ,(iter-atom fun f #t) . ,(map (lambda (x) (iter-atom fun x #f)) xs)))
        ((f xs ...)
         (cons (iter-atom fun f #t)
               (map (lambda (x) (iter-atom fun x #f)) xs)))
        (else (error "to-functions iter-apply match statement exhausted" expr))))
    (define (tofunctions-iter fun expr) (match expr
        (('bruijn . _) expr)
        (('lambda . _) (iter-atom fun expr #f))
        (('##qualified-lambda . _) (iter-atom fun expr #f))
        (('case-lambda . _) (iter-atom fun expr #f))
        (('##qualified-case-lambda . _) (iter-atom fun expr #f))
        (('continuation . _) (iter-atom fun expr #f))
        (('quote . _) (iter-atom fun expr #f))
        (('##inline . _) (iter-atom fun expr #f))
        ((_ . _) (iter-apply fun expr))
        (else (iter-atom fun expr #f))))
    (define (iter-declare d)
      (match d
        (('##foreign.declare . _) d)
        (('##vcore.declare f l)
         (list '##vcore.declare  f (cadr (tofunctions-iter f l))))))
    (define (multi-partition f n l)
      (let loop ((ret (make-list n '())) (l l))
        (if (null? l)
            (apply values (map reverse ret))
            (begin
              (let ((split (f (car l))))
                (list-set! ret split (cons (car l) (list-ref ret split))))
              (loop ret (cdr l))))))
    #;(parameterize ((current-output-port (current-error-port)))
      (for-each pretty-print exprs)
    )
    (call-with-values
      (lambda ()
        (multi-partition
          (lambda (e)
            (match e
              (('##foreign.declare . _) 1)
              (('##vcore.declare . _) 1)
              (else 0)))
          2 exprs))
      (lambda (globals declares)
        (let ((toplevels (map (lambda (e) (tofunctions-iter "global" e)) globals))
              (declares (map iter-declare declares)))
          (list static-environments literal-table (map cdr foreign-functions) (map cdr intrinsics) functions qualified-functions declares toplevels)))))
)
