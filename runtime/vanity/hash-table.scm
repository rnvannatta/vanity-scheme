(define-library (vanity hash-table)
  ...
  (export
    current-hash
    make-eq-hash-table make-eqv-hash-table make-hash-table
    hash-table? hash-table-ref hash-table-set! hash-table-delete!
    hash-table-size
  ...
  (define make-hash-table 
    (case-lambda
      ...
      ((equal) (make-hash-table equal (if (eq? equal eq?) current-hash #f) #f (not (eq? equal eq?))))
      ((equal hash)
  ...
  (define make-eq-hash-table
    (case-lambda
      (() (make-hash-table eq? current-hash #f #f))
      ((weakness) (make-hash-table eq? current-hash weakness #f))))
  (define make-eqv-hash-table make-eq-hash-table)
